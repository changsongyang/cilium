...
func (m *metadata) upsert(prefix string, lbls labels.Labels, src source.Source, name types.ResourceID) {
	...
	m.m[prefix][name] = newLabelsWithSource(l, src)
	m.Unlock()
}
...
func (ipc *IPCache) RemoveLabelsExcluded(
	...
	for _, ip := range oldSet {
		if _, ok := toExclude[ip]; !ok {
			ipc.removeLabels(ip, lbls, src, name)
		}
	}
}
...
func (ipc *IPCache) removeLabels(prefix string, lbls labels.Labels, src source.Source, name types.ResourceID) {
